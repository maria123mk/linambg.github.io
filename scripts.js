...
function checkDuePayments() {
    ...
    clients.forEach(client => {
        if (client.dueDate <= today) {
            const tableRows = document.getElementById('clients-table').getElementsByTagName('tbody')[0].rows;
            for (let row of tableRows) {
                if (row.cells[0].textContent === client.name) {
                    highlightDueToday(row, client.dueDate);
                }
            }
        }
    });
